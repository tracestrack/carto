...
tree.functions = {
    ...
    mix: function (color1, color2, weight) {
        ...
        var mix = null;
        if (perceptual) {
            var normalized = _.map(rgb, function(x) { return x / 255; });
            hsluv.rgb_r = normalized[0];
            hsluv.rgb_g = normalized[1];
            hsluv.rgb_b = normalized[2];
            hsluv.rgbToHsluv();
            mix = [hsluv.hsluv_h, hsluv.hsluv_s / 100, hsluv.hsluv_l / 100];
        }
        else {
            mix = chroma(rgb).hsl();
        }
